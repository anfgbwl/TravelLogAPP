...
- fetchBucketList 함수 내 tableView reload 핸들러 미작동 문제
  ...
  ```
- Task 추가 시 테이블 뷰 리로드되지 않는 문제
  ```
  fetchCategory의 완료 클로저 내에서 fetchBucketList 메서드를 호출하고, 버킷 리스트를 가져오는 동안 카테고리 데이터를 로드하도록 변경
  ```
- 모든 Task 삭제 시 카테고리가 N/A로 유지되어 보이는 문제
  ```
  category가 없는 경우 테이블뷰 헤더 title을 지정하지 않게 else 문 삭제
  ```
